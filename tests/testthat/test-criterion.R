...
test_that("Can calculate degree corrected MDL", {
  ...
  p1 <- c(rep(1,2), rep(2,2))
  c1 <- calculate_dl(g1, p1, degree_correction = TRUE)
  ...
  p2 <- c(1,2)
  c2 <- calculate_dl(g2, p2, degree_correction = TRUE)
  ...
  g3 <- add.edges(g3, c(1,2))
  p3 <- c(1,2)
  c3 <- calculate_dl(g3, p3, degree_correction = TRUE)
